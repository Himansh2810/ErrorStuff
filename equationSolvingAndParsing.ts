import { boolean, derivative, evaluate, string } from "mathjs";
...
class EquationParser {
  ...
  public identifyTokens(token: string): {valid:boolean,type:string}{
    const exp = {
      start: ":",
      variable: "x",
      lp: "(",
  ...
  public validTokenPair(tokenPair:Array<string>){
  ...
  public verifyToken(i: number) {
    try {
      ...
      let p2: boolean,
        p3: boolean,
        p1 = this.identifyTokens(expression[i]).valid;
      if (!p1) {
        let temp = expression[i] + expression[i + 1];
        p2 = this.identifyTokens(temp).valid;

        if (!p2 && this.identifyTokens(expression[i + 1]).valid) {
          throw new Error(
        ...
        if (!p2) {
          temp += expression[i + 2];
          p3 = this.identifyTokens(temp).valid;
          if (!p3 && this.identifyTokens(expression[i + 2]).valid) {
            throw new Error(
              `Unexpected token ${temp.slice(
                0,
                temp.length - 1
...
const res = new EquationParser("2*x+++(()sin(x)").createTokens();
